{ pkgs, lib, ... }:
{
  ...
  programs.astroid = {
    enable = true;
    externalEditor = "urxvt -e nvim -c 'set ft=mail' '+set fileencoding=utf-8' '+set ff=unix' '+set enc=utf-8' '+set fo+=w' %1";
  };

  systemd.user.services.email-fetch = {
    Unit = {
      Description = "Fetch email and index.";
    };
    Service = 
      let
